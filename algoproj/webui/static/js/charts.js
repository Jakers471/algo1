...
(function () {
  ...
  // Candlestick with BUY/SELL order markers (algoproj chart view).
  function candles(eln, { candles, marks }) {
    const c = init(eln);
    const x = candles.map(r => r[0]);
    const ohlc = candles.map(r => [r[1], r[2], r[3], r[4]]); // open, close, low, high
    const byTime = {}; x.forEach((t, i) => (byTime[t] = i));
    const buys = [], sells = [];
    (marks || []).forEach(m => {
      const i = byTime[m.t]; if (i == null) return;
      const lo = candles[i][3], hi = candles[i][4];
      if (m.side === 'buy') buys.push({ value: [i, lo] });
      else sells.push({ value: [i, hi] });
    });
    c.setOption({
      tooltip: tooltip({ trigger: 'axis', axisPointer: { type: 'cross' } }),
      grid: { left: 56, right: 16, top: 16, bottom: 56, containLabel: true },
      xAxis: catAxis(x, { boundaryGap: true, axisLabel: { interval: Math.floor(x.length / 8), formatter: v => String(v).slice(0, 10) } }),
      yAxis: valAxis({ extra: { scale: true } }),
      dataZoom: [{ type: 'inside' }, { type: 'slider', height: 18, bottom: 18, backgroundColor: C.panel, borderColor: C.border, fillerColor: 'rgba(45,212,191,.12)', textStyle: { color: C.text3 } }],
      series: [
        { type: 'candlestick', data: ohlc, itemStyle: { color: C.green, color0: C.red, borderColor: C.green, borderColor0: C.red } },
        { type: 'scatter', data: buys, symbol: 'triangle', symbolSize: 11, symbolOffset: [0, 12], itemStyle: { color: C.green }, z: 5, tooltip: { show: false } },
        { type: 'scatter', data: sells, symbol: 'triangle', symbolRotate: 180, symbolSize: 11, symbolOffset: [0, -12], itemStyle: { color: C.red }, z: 5, tooltip: { show: false } }
      ]
    });
    return c;
  }
  ...
  function equityDD(eln, { dates, equity, dd }) {
    ...
    c.setOption({
      ...
      grid: [{ left: 60, right: 18, top: 14, height: '58%', containLabel: true },
             { left: 60, right: 18, top: '74%', height: '18%', containLabel: true }],
      xAxis: [
        { type: 'category', data: dates, boundaryGap: false, gridIndex: 0, axisLine: { lineStyle: { color: C.border } }, axisTick: { show: false }, axisLabel: { show: false }, splitLine: { show: false } },
        { type: 'category', data: dates, boundaryGap: false, gridIndex: 1, axisLine: { lineStyle: { color: C.border } }, axisTick: { show: false }, axisLabel: { color: C.text3, fontSize: 11, interval: Math.floor(dates.length / 8), formatter: v => String(v).slice(0, 7) }, splitLine: { show: false } }
      ],
      yAxis: [
        { type: 'value', gridIndex: 0, scale: true, axisLine: { show: false }, axisTick: { show: false }, axisLabel: { color: C.text3, fontSize: 11, formatter: v => '$' + (v / 1000).toFixed(0) + 'k' }, splitLine: { lineStyle: { color: C.grid } } },
        { type: 'value', gridIndex: 1, max: 0, axisLine: { show: false }, axisTick: { show: false }, axisLabel: { color: C.text3, fontSize: 11, formatter: v => v.toFixed(0) + '%' }, splitLine: { lineStyle: { color: C.grid } } }
      ],
      series: [
        { name: 'equity', type: 'line', data: equity, xAxisIndex: 0, yAxisIndex: 0, smooth: true, showSymbol: false, lineStyle: { width: 1.6, color: C.accent }, areaStyle: { color: areaGrad(C.accent) } },
        { name: 'drawdown %', type: 'line', data: dd, xAxisIndex: 1, yAxisIndex: 1, showSymbol: false, lineStyle: { width: .8, color: C.red }, areaStyle: { color: 'rgba(248,113,113,.22)' } }
      ]
    });
    return c;
  }
